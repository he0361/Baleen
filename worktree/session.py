...
from Baleen.worktree.models import WorktreeSession
...
def _session_path(baleen_dir: Path) -> Path:
    return baleen_dir / SESSION_FILENAME
...
def load_worktree_session(baleen_dir: Path) -> WorktreeSession | None:
    path = _session_path(baleen_dir)
    if not path.exists():
        return None
    try:
