...
from Baleen.conversation import ConversationManager, ToolResultBlock, ToolUseBlock
from Baleen.conversation import ThinkingBlock as ConvThinkingBlock
from Baleen.memory.auto_memory import MemoryManager
from Baleen.permissions import (
    Decision,
    PermissionChecker,
    PermissionMode,
)
from Baleen.hooks import HookContext, HookEngine, ToolRejectedError
from Baleen.hooks.engine import HookNotification
from Baleen.prompts import build_environment_context, build_plan_mode_reminder, build_system_prompt
from Baleen.tools import ToolRegistry
from Baleen.tools.base import (
    MAX_OUTPUT_CHARS,
    StreamEnd,
    StreamEvent,
    TextDelta,
...
class Agent:
    ...
    def _transcript_path(self) -> str:
        if self.session_id:
            return str(Path(self.work_dir) / ".Baleen" / "sessions" / f"{self.session_id}.jsonl")
        return ""
    ...
    def _get_plan_path(self) -> Path:
        ...
        plans_dir = Path(self.work_dir) / ".Baleen" / "plans"
        ...
        if self.permission_checker:
            ...
            if decision.effect == "ask":
                ...
                if response == PermissionResponse.ALLOW_ALWAYS:
                    from Baleen.permissions.rules import Rule, extract_content
    ...
    def _maybe_persist_or_truncate(self, tool_use_id: str, text: str) -> str:
        from Baleen.context.manager import (
