...
def is_coordinator_mode(enable_flag: bool = False) -> bool:
    if not enable_flag:
        return False
    val = os.environ.get("BALEEN_COORDINATOR_MODE", "").lower()
...
def match_session_mode(
    ...
    if session_is_coordinator:
        os.environ["BALEEN_COORDINATOR_MODE"] = "1"
    else:
        os.environ.pop("BALEEN_COORDINATOR_MODE", None)
...
def get_coordinator_system_prompt(agent_catalog: list[tuple[str, str]] | None = None) -> str:
    ...
    return """You are Baleen, an AI assistant that orchestrates software engineering tasks across multiple workers.
...
def get_coordinator_user_context(
    ...
    if worker_tools is None:
        from Baleen.agents.tool_filter import IN_PROCESS_TEAMMATE_ALLOWED_TOOLS
