...
from Baleen.commands import (
    CommandContext,
    CommandRegistry,
    complete,
    parse_command,
)
from Baleen.commands.completion import CompletionPopup
from Baleen.commands.handlers import register_all_commands
from Baleen.config import MCPServerConfig, ProviderConfig
from Baleen.hooks import HookContext, HookEngine, load_hooks
from Baleen.conversation import ConversationManager, Message
from Baleen.mcp import MCPManager
from Baleen.memory import (
    MemoryManager,
    Session,
    SessionManager,
...
from Baleen.permissions import (
...
from Baleen.agents.task_manager import TaskManager
from Baleen.agents.trace import TraceManager
from Baleen.agents.notification import inject_task_notifications
from Baleen.commands.handlers.tasks import create_tasks_command
from Baleen.skills.executor import SkillExecutor
from Baleen.skills.loader import SkillLoader
from Baleen.commands.handlers.skill_register import register_skill_commands
from rich.text import Text as RichText
from textual.theme import Theme
from Baleen.cache import FileCache
from Baleen.tools import ToolRegistry, create_default_registry
from Baleen.tools.agent_tool import AgentTool
from Baleen.tools.ask_user import AskUserEvent, AskUserTool
from Baleen.tools.impl.tool_search import ToolSearchTool
from Baleen.tools.load_skill import LoadSkill
from Baleen.worktree.cleanup import start_stale_cleanup_task
from Baleen.worktree.manager import WorktreeManager
from Baleen.commands.handlers.worktree import create_worktree_command
from Baleen.teammate_tree import TeammateTree
...
_SKIP_DIRS = {".git", "node_modules", ".venv", "__pycache__", ".Baleen", "build", ".gradle"}
...
class ChatInput(TextArea):
    ...
    def load_history(self, work_dir: str) -> None:
        self._history_file = Path(work_dir) / ".Baleen" / "history"
...
class BaleenApp(App):
    ...
    def __init__(
        ...
        compact_banner: bool = True,
    ) -> None:
        super().__init__(driver_class=driver_class)
        self._compact_banner = compact_banner
    ...
    def _make_banner(self, model: str = "", work_dir: str = "") -> RichText:
        from Baleen.branding import make_banner
        return make_banner(model, work_dir, compact=self._compact_banner)
    ...
    def on_mount(self) -> None:
        self.query_one('#title-bar').styles.height = 8 if self._compact_banner else 13
        self.register_theme(_BALEEN_THEME)
        self.theme = "Baleen"
        if len(self.providers) == 1:
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        checker = PermissionChecker(
            ...
            rule_engine=RuleEngine(
                user_rules_path=home / ".Baleen" / "permissions.yaml",
                project_rules_path=Path(work_dir) / ".Baleen" / "permissions.yaml",
                local_rules_path=Path(work_dir) / ".Baleen" / "permissions.local.yaml",
            ),
        ...
        from Baleen.filehistory import FileHistory
        ...
        from Baleen.tools.exit_plan_mode import ExitPlanModeTool
        ...
        # --- Worktree 系统初始化 ---
        from Baleen.config import WorktreeConfig
        ...
        from Baleen.tools.enter_worktree import EnterWorktreeTool
        from Baleen.tools.exit_worktree import ExitWorktreeTool
        ...
        # --- Agent 团队系统初始化 ---
        from Baleen.teams.manager import TeamManager
        from Baleen.tools.team_create import TeamCreateTool
        from Baleen.tools.team_delete import TeamDeleteTool
        ...
        from Baleen.commands.handlers.trace import create_trace_command
        ...
        from Baleen.tools.synthetic_output import SyntheticOutputTool
    ...
    def on_chat_input_slash_menu_update(self, event: ChatInput.SlashMenuUpdate) -> None:
        if not self.query(CompletionPopup):
            return
        popup = self.query_one(CompletionPopup)
    ...
    async def _prefetch_relevant_memories(self, query: str) -> str:
        ...
        async def selector(system_prompt: str, user_message: str) -> str:
            from Baleen.tools.base import StreamEnd, TextDelta
    ...
    async def _show_plan_approval(self) -> None:
        from Baleen.plan_dialog import InlinePlanWidget
    ...
    def on_inline_plan_widget_responded(
        ...
        from Baleen.plan_dialog import InlinePlanWidget, PlanChoice
    ...
    async def _handle_askuser(self, event: AskUserEvent) -> None:
        from Baleen.askuser_dialog import InlineAskUserWidget
    ...
    async def _handle_permission_request(self, request: PermissionRequest) -> None:
        from Baleen.permission_dialog import InlinePermissionWidget
    ...
    def _show_error(self, text: str) -> None:
        chat = self.query_one("#chat-area", VerticalScroll)
        error_widget = Static(f"[!] {text}", classes="message error-message", markup=False)
