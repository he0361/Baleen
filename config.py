...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    home = Path.home()
    candidates = [
        home / ".Baleen" / "config.yaml",
        cwd / ".Baleen" / "config.yaml",
        cwd / ".Baleen" / "config.local.yaml",
    ]
    ...
    if merged is None:
        raise ConfigError(
            "No config file found. Expected .Baleen/config.yaml "
            "in project or ~/.Baleen/config.yaml"
        )
    return merged
