...
from Baleen.config import ConfigError, load_config
from Baleen.hooks import HookConfigError, HookEngine, load_hooks
from Baleen.permissions import PermissionMode


def main() -> None:
    # 先确保 .Baleen/ 目录存在，否则下面写 debug.log 会因目录不存在而崩溃
    Path(".Baleen").mkdir(parents=True, exist_ok=True)
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s %(name)s %(message)s",
        filename=".Baleen/debug.log",
        filemode="w",
    )

    parser = argparse.ArgumentParser(prog="Baleen", description="Baleen AI coding assistant")
    parser.add_argument(
        "--mode",
...
async def _run_prompt(config, permission_mode, hook_engine, prompt: str) -> None:
    from Baleen.agent import Agent
    from Baleen.client import create_client, resolve_context_window
    from Baleen.conversation import ConversationManager
    from Baleen.memory.instructions import load_instructions
    ...
    from Baleen.tools import create_default_registry
